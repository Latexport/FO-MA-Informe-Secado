document.addEventListener("DOMContentLoaded", () => {
  ...
  async function obtenerNumeroFila(sheet, context) {
    try {
      ...
      if (dataRange.load.rowCount > 0) {
        // Devolver la última fila + 1
        return dataRange.load.rowCount + 1;
      } else {
        // Si no hay datos, devolver la primera fila
        return 1;
      }
    } catch (error) {
  ...
  async function agregarDatosExcel(nombreHoja, data) {
    try {
      await Excel.run(async (context) => {
        ...
        console.log("Número de fila obtenido:", fila);
  
        // Ajustar la ubicación del rango según tus necesidades
        const columnaLetra = obtenerLetraColumnaDesdeNumero(data[0].length);
        const rangoDireccion = `A${fila}:${columnaLetra}${fila}`;
        
        // Obtener el rango en la hoja de cálculo
        const dataRange = sheet.getRange(rangoDireccion);
        
        // Establecer los valores en el rango
        dataRange.values = data;
  
        return context.sync();
      });
    } catch (error) {
      console.error("Error al agregar datos a Excel:", error);
      throw error;
    }
  }
